"""FastAPI app exposing the Torrey Pines shaker over lab status spec v1.2.
...
def create_app(
    *,
    ...
    async def control_set_temperature(
        ...
        gate = await service.evaluate_action_gate("shake.set_temperature")
        if gate is not None:
            raise _ClaimResponseException(status_code=412, payload=gate)
        try:
    ...
    async def control_set_speed(
        ...
        gate = await service.evaluate_action_gate("shake.set_speed")
        if gate is not None:
            raise _ClaimResponseException(status_code=412, payload=gate)
        try:
    ...
    async def control_shake_start(
        ...
        gate = await service.evaluate_action_gate(
            "shake.start", wait_for_temperature=req.wait_for_temperature
        )
        if gate is not None:
            raise _ClaimResponseException(status_code=412, payload=gate)
        try:
